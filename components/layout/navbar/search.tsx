...
import {
  createUrl,
  getUniqueDiamondClarities,
  getUniqueDiamondColours,
  getUniqueDiamondTypes,
  getUniqueMetals,
  getUniqueRetailers,
  RingType,
} from "@/utils";
...
export default function Search() {
  ...
  const ringType = (searchParams?.get("ringType") as RingType) || "engagement";

  const uniqueRetailers = getUniqueRetailers(ringType);
  const uniqueDiamondTypes = getUniqueDiamondTypes(ringType);
  const uniqueDiamondColours = getUniqueDiamondColours(ringType);
  const uniqueDiamondClarities = getUniqueDiamondClarities(ringType);
  const uniqueMetals = getUniqueMetals(ringType);
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Preserve ringType when navigating
    if (ringType) {
  ...
  const filter = (field: FilterField, value: string) => {
    ...
    // Preserve ringType when filtering
    if (ringType) {
      newParams.set("ringType", ringType);
    }
  ...
  const sort = (field: SortField | "select", direction: "asc" | "desc") => {
    ...
    // Preserve ringType when sorting
    if (ringType) {
      newParams.set("ringType", ringType);
    }
